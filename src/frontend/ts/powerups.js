define(["require", "exports", "./util", "./eval"], function (require, exports, util_1, eval_1) {
    "use strict";
    ...
    exports.MultiplierPowerup = MultiplierPowerup;
    class NewVarPowerup extends MultiplierPowerup {
        constructor(varname, multiplier = 2) {
            super("NewVar=" + varname, varname + "<span style='position:absolute;left:4px;color:goldenrod'>" +
                "*</span>", (inv) => util_1.isMember(eval_1.identifiers(inv), varname), multiplier, (lvl) => true, "use '" + varname + "' in an expression");
    ...
    class VarOnlyPowerup extends MultiplierPowerup {
        constructor(multiplier = 2) {
            super("var only", "<span style='position: absolute; left:13px'>1</span>" +
                "<span style='position: absolute;color:red; left:10px'>&#10799;</span>", (inv) => util_1.setlen(eval_1.literals(inv)) === 0, multiplier, (lvl) => true, "don't use constants");
        }
    }
    exports.VarOnlyPowerup = VarOnlyPowerup;
    class UseXVarsPwup extends MultiplierPowerup {
        constructor(nvars, multiplier = 2) {
            super("NVars=" + nvars, nvars + "V", (inv) => util_1.setlen(eval_1.identifiers(inv)) === nvars, multiplier, (lvl) => lvl.variables.length >= nvars && lvl.variables.length != 1, "you use " + nvars + " variable(s)");
        }
    }
    class UseOpsPwup extends MultiplierPowerup {
        constructor(ops, html, name, multiplier = 2) {
            super("Use ops: " + ops, html, (inv) => {
                let inv_ops = eval_1.operators(inv);
                for (let i in ops) {
    ...
    class PowerupSuggestionAll {
        ...
        clear(lvl) {
            ...
            if (util_1.Args.get_use_new_var_powerup()) {
            ...
            for (let p of this.all_pwups) {
                ...
                if (newM > mPwup.mult) {
                    let fn = () => {
                        ...
                        let l = util_1.label({ "at": "left center", "of": mPwup.element }, "Went up to " + newM + "X !", "right");
                        let t = setTimeout(() => { util_1.removeLabel(l); }, delay);
                        this.timers.push(t);
                        this.lbls.push(l);
                    };
                    fn();
    ...
    exports.PowerupSuggestionAll = PowerupSuggestionAll;
    class PowerupSuggestionFullHistory {
        ...
        getPwups() {
            if (this.type === "lru") {
                ...
                util_1.assert(this.type === "lfu");
                return this.sortFreq.slice(0, this.nDisplay).map(([fst, snd]) => snd);
            }
        }
    }
    exports.PowerupSuggestionFullHistory = PowerupSuggestionFullHistory;
    class PowerupSuggestionFullHistoryVariableMultipliers extends PowerupSuggestionFullHistory {
        ...
        computeOrders() {
            ...
            this.shown = {};
            for (var i = 0; i < util_1.min(this.nDisplay, lst.length); i++) {
                this.shown[lst[i][1].id] = true;
            }
            if (this.lbl) {
                clearTimeout(this.timer);
                util_1.removeLabel(this.lbl);
                this.lbl = null;
            }
        }
        invariantTried(inv) {
            ...
            for (let pwup of this.actual) {
                ...
                if (this.shown[pwup.id] && age > 7 && this.lbl == null) {
                    this.lbl = util_1.label({ "at": "left center", "of": pwup.element }, "You haven't tried <br> this in a <br> while!", "right");
                    this.timer = setTimeout(() => { util_1.removeLabel(this.lbl); }, 5000);
                }
            }
        }
    }
    ...
    class TwoPlayerVarOnlyPowerup extends TwoPlayerMultiplierPowerup {
        constructor(playerNum, multiplier = 2) {
            super(playerNum, "var only", "<span style='position: absolute; left:13px'>1</span>" +
                "<span style='position: absolute;color:red; left:10px'>&#10799;</span>", (inv) => util_1.setlen(eval_1.literals(inv)) === 0, multiplier, (lvl) => true, multiplier + "X if you don't use constants");
        }
    }
    class TwoPlayerUseXVarsPwup extends TwoPlayerMultiplierPowerup {
        constructor(playerNum, nvars, multiplier = 2) {
            super(playerNum, "NVars=" + nvars, nvars + "V", (inv) => util_1.setlen(eval_1.identifiers(inv)) === nvars, multiplier, (lvl) => lvl.variables.length >= nvars && lvl.variables.length !== 1, multiplier + "X if you use " + nvars + " variable(s)");
        }
    }
    ...
    exports.TwoPlayerPowerupSuggestionFullHistory = TwoPlayerPowerupSuggestionFullHistory;
});
