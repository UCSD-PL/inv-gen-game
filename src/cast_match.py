...
class Var:
    def __init__(s, name, optional=False):
        s._name = name
        s._optional = optional

    def __hash__(s):
        return -1 * hash(s._name)

    def __eq__(s, other):
        if not isinstance(other, Var):
            return False
        return s._name == other._name

    def __repr__(s):
        return "Var({})".format(s._name)
    ...
    def instantiate(s, suffix):
        return Var(s._name + suffix, s._optional)
