...
def propagate_sp(bbs):
    ...
    def untouched(preds, clobbered_vars):
      return set([x for x in preds if len(clobbered_vars.intersection(expr_read(x))) == 0])
    ...
    transformer_m = {
        ...
        AstAssignment:  lambda stmt, inS:   untouched(inS, set([str(stmt.lhs)])).union(assignment_preds(stmt))
    }

    def union_f(sets):
        something_sets = filter(lambda x:  x != None, sets)
        if (len(something_sets)) == 0:  return None
        return reduce(lambda x,y:   x.intersection(y), something_sets[1:], something_sets[0])
